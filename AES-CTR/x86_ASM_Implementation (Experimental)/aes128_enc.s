...
wrong_argc:             #74 characters
    .ascii "Syntax: aes128_enc [input filename] [16-byte key file] [output filename]\n\0"
...
open_input_file_error:  #37 characters
    .ascii "Error while opening the input file.\n\0"
...
.section .text
    ...
    .equ OPEN_INPUT_FILE_ERROR_MESSAGE_LEN, 37
...
main_open_file_in:
    ...
    cmpl $0, %eax
    jl open_file_in_error_end_program
...
main_nonce_generation:
    #Nonce generation with 32-bit time
    pushl $256
    pushl $16
    pushl $NONCE
    call rand
...
open_file_in_error_end_program:
    ...
    movl $SYS_WRITE, %eax
    movl $STDOUT, %ebx
    movl $open_input_file_error, %ecx
    movl $OPEN_INPUT_FILE_ERROR_MESSAGE_LEN, %edx
    int $LINUX_SYSCALL
